...
export const RULES = {
  ...
  PENALTY_HOURS: 4,
  /** Two 429 trips on 2026-08-12 found the real ceiling below the published 2 RPS. */
  COOLING_RPS: 1.7,
  NORMAL_RPS: 2.0,
...
export function windowDecision({
  ...
  const hourly = Math.max(0, Math.min(rules.HOURLY_CAP, rules.HOURLY_TARGET - last1h));
 ...
 *  Anything unparseable reads as null rather than as 0 — epoch 0 would look like a penalty
 *  served in 1970 and would silently wave every future window through. */
export function readPenaltyAt(readFile, file = PENALTY_FILE) {
  let raw;
  try {
    raw = readFile(file);
  } catch {
    return null; // no marker is the normal case
  }
