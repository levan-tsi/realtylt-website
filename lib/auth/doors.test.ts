...
describe("getAuthDoors", () => {
  ...
  it("reads the project's live settings, not a build-time flag", async () => {
    ...
    expect(await getAuthDoors()).toEqual({ signupOpen: true, google: true });
    const url = (globalThis.fetch as ReturnType<typeof vi.fn>).mock.calls[0][0];
    expect(String(url)).toBe("https://project.supabase.co/auth/v1/settings");
  });

  // The live state of the RealtyLT project on 2026-08-18, and the reason this module exists.
  it("reports both doors shut when signup is disabled and google is off", async () => {
    globalThis.fetch = settings({ disable_signup: true, external: { google: false } });
    expect(await getAuthDoors()).toEqual({ signupOpen: false, google: false });
  });
