 ...
 * frames): ours must not sit on them. Where Google names the same PLACE (none of ours: Google
 * names cities, we name counties and boroughs), Google's would carry it. */
export const GOOGLE_CITY_LABELS: readonly AreaLabel[] = [{ id: "g-new-york", text: "New York", tier: "county", lat: 40.7128, lng: -74.006 }];
 ...
 * between two names; `edge`: from the window's edges. */
export function placeLabels(
  items: readonly LabelItem[],
  obstacles: readonly Box[],
  vp: { width: number; height: number },
  opts: { pad?: number; gap?: number; edge?: number } = {},
): PlacedLabel[] {
