...
export function chipStateStyles(s: { active: boolean; saved: boolean; spokenFor: boolean }): { outer: string; face: string } {
  ...
  // The resting chip reads its colours from the page (round 53): black on the day map, moonlight
  // on the night map, where a black chip on a dark basemap had nothing to stand out against.
  return { outer: "--chip-bg:var(--rlt-chip-bg,#000)", face: "background:var(--chip-bg);color:var(--rlt-chip-ink,#fff);box-shadow:0 2px 8px rgb(0 0 0/.3)" };
}
