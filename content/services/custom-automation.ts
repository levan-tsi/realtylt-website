...
export const customAutomation: Service = {
  ...
  whatItIs: [
    "It is the version of all of this that gets built around your business, instead of the other way round. Every team has one step that no product on the market fixes, because it is specific to how they work. That step is usually the thing quietly capping their growth.",
    "The parts are the same ones behind every other service here. There are agents that talk, pipelines that enrich, and systems that qualify, schedule, sync and file. What changes is the way they are put together. It gets orchestrated in n8n and wired into the tools you already run.",
  ],

  howItWorks: [
    {
      title: "Find the step that is actually capping you",
      body: "Not the one that is annoying. The one that is limiting. They are often different, and only one of them is worth building for.",
    },
    {
    ...
    },
    {
      ...
      body: "Orchestrated in n8n, wired to your systems, logged on every run, versioned so a change can be rolled back. It fails loudly rather than quietly. That is because the most common ending for a chain like this is silence, and silence looks exactly like having nothing to do.",
    },
    {
      title: "Hand it over so somebody else could pick it up",
      body: "A written description of what it does, kept with the thing itself. It is in the language of the business rather than of the software. With it come logins and an environment that are yours. That page costs an hour at the end of a build, and it is the difference between the next change being a change and being a rebuild.",
    },
  ],
  ...
  useCases: [
    {
      ...
      body: "Specific to how you work, manual, and the reason hiring starts to feel like the only option. Sometimes it is. The useful question first is how many other businesses would recognise the description. That is because if the answer is thousands, there is probably a product.",
    },
    {
    ...
    },
    {
      ...
      body: "When doubling the work means doubling the people, the thing holding you back is manual process. Automating it changes what growth costs.",
    },
  ],

  limits: [
    "It does not build what you cannot describe. The honest test is whether you could explain the task to a competent new hire. If you cannot, deciding how the work should run is the first job.",
    "It does not replace your stack. Automation sits between the systems you already run and connects them. So nothing here is a reason to move your CRM.",
    ...
    "It does not take the person out of the steps that need one. A step that guesses where it ought to have asked will, on some future day, be confidently wrong to a client's face.",
    ...
    "It does not retire itself. A build with no review date runs until something outside it breaks it. So a vendor decides when you stop, rather than you.",
  ],
  ...
  faqs: [
    ...
    },
    {
      ...
      a: "Because an off-the-shelf product solves the average version of a problem, and the step capping your growth is usually not average. n8n orchestrates custom chains and runs your own logic. It can also be self-hosted, so the data stays in your environment.",
    },
    {
      q: "How do you decide what to build first?",
      a: "By how contained the worst case is, then by whether the rule is actually settled, and only then by the size of the saving. That order is deliberate. On a sample of 1,471 technology projects worth 241 billion dollars, 17 percent landed in a fat right hand tail. A thin-tailed distribution would have put under one percent there. So an expected payback describes the middle of the range rather than what you are exposed to. If you have not mapped any of this yet, the AI audit is the place to start.",
    },
    {
      q: "What happens when the software it connects to changes?",
      a: "Usually nothing, occasionally something. The awkward middle case is a change that is not officially a breaking change at all. Vendors are allowed to add new values to a field without changing an API version, and they document that they will. Which is why the thing to specify in any build is what it does when it meets something unfamiliar. And the only good answers involve stopping and telling a person.",
    },
    {
