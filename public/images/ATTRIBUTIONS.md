...
Two folders are NOT stock photography and must not be swapped for it:
...
- `public/images/mls/*.svg` — the One Key MLS mark, One Key MLS's trademark, self-hosted from
  their own CDN by `scripts/fetch-onekey-logo.mjs`. It is displayed as part of the required
  IDX attribution (`components/idx/MlsAttribution.tsx`), not as decoration, so it may not be
