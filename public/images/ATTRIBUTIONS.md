...
| public/images/home-night-cover.webp | The real map's load cover at a laptop's width, the default since round 57.6: the night map's own first frame, drawn by `scripts/make-map-cover.mjs --look=night` from the map's own hero camera. The relief and water are the elevation data credited under `public/geo/` below (its red and green channels only), shaded in the tones of the live map's night grade; built-up land is told by the density of our own listings (`/api/lights`); the lights are our own listing coordinates drawn with the live layer's own glyph and planner. No night-light imagery (the asset's blue channel, NASA) and no map imagery of anyone's is in it | RealtyLT | Our own artwork | -- |
