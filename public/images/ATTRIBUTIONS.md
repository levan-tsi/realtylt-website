...
| public/images/editorial/ships-barograph.jpg | Ships barograph — a recording barometer, its paper drum ruled in three languages across the days of the week (Donderdag/Thursday/Jeudi, Vrijdag/Friday/Vendredi legible) with the pen arm resting on it and the aneroid capsule stack beside it. SPENT: cover of the Singularity flagship. Round 43. Licence, title and photographer read on the rendered Flickr page in Chromium (`scripts/_scratch-h-owner.mjs`), matched on the licence URL | Piet Krom | CC0 1.0 | [source](https://www.flickr.com/photos/192185971@N02/54714839776) |
| public/images/editorial/punched-tape.jpg | 0413 punched tape — two strips of punched paper tape on black felt, one inverted, with QUALITY TESTED printed along them and legible through the perforations. SPENT: cold-open field of the Singularity flagship. Round 43 | crabchick | BY 2.0 | [source](https://www.flickr.com/photos/62327186@N00/2774206139) |
| public/images/editorial/wind-tunnel-model.jpg | General Electric Fan-In-Fuselage Model in the 40x80 Foot Wind Tunnel at NASA Ames, May 14th 1961 — a yellow full-scale test model on trestles at the mouth of the tunnel. SPENT: plate one of the Singularity flagship. Round 43. The Flickr page states "Original from NASA. Digitally enhanced by rawpixel" and the reposter licenses it CC BY 2.0; the underlying NASA photograph is a US government work. Both halves of that provenance are recorded here rather than only the convenient one | Free Public Domain Illustrations by rawpixel, from NASA | BY 2.0 | [source](https://www.flickr.com/photos/153584064@N07/31425293957) |
...
| public/images/home-night-poster.webp | The home page's first screen with no JavaScript and no WebGL: one frame of the night-flight scene's own establishing shot, rendered from the running site by `scripts/make-night-poster.mjs` (round 54; since round 57 shot from the real map's hero camera; since round 57.2 the real map has its own covers, below, and this still is the night ground's and the dusk cover's light layer). The lights are our own listing coordinates; the land is the elevation data credited under `public/geo/` below | RealtyLT | Our own artwork | -- |
| public/images/home-night-cover.webp | Since round 57.13 the load cover of the home page's MapLibre night map at a laptop's width (1583 x 1000, the laptop's 1425 x 900 map box at 1.11x): a frame of that map itself, rendered by `scripts/make-ml-cover.mjs` from the running site (the territory shot with every tile drawn, our lights planned and drawn by the live layer, everything else hidden), so the dissolve into the live map is a change of nothing. The map in it is a Produced Work of OpenStreetMap data (© OpenStreetMap contributors, ODbL) in OpenMapTiles vector tiles (© OpenMapTiles) served by OpenFreeMap, with the relief from the AWS Open Data terrain tiles (Mapzen/Tilezen: USGS 3DEP, SRTM, GMTED2010; NOAA ETOPO1); the style (colours, hairlines, moonlight) is ours (`components/home/ml/style.ts`); the lights are our own listing coordinates. The attribution those sources ask for is on the page with it: the map's corner ("© OpenStreetMap contributors, © OpenMapTiles, OpenFreeMap") and the footer (`components/site/SceneCredit.tsx`) | RealtyLT; map data © OpenStreetMap contributors, © OpenMapTiles | Our own render of open data (ODbL produced work, attributed) | [source](https://www.openstreetmap.org/copyright) |
| public/images/home-night-cover-tall.webp | The same MapLibre cover for the phone (780 x 1688, the phone's 390 x 844 map box at 2x), from the phone's own territory camera, by `scripts/make-ml-cover.mjs` | RealtyLT; map data © OpenStreetMap contributors, © OpenMapTiles | Our own render of open data (ODbL produced work, attributed) | [source](https://www.openstreetmap.org/copyright) |
| public/images/home-g3d-cover.webp | Google's 3D map's load cover at a laptop's width (only when `NEXT_PUBLIC_HOME_MAP=g3d`; until round 57.13 it was `home-night-cover.webp`, moved here unchanged when that name became the MapLibre map's cover): the night map's own first frame, drawn by `scripts/make-map-cover.mjs --look=night` from Google's map's hero camera. The relief and water are the elevation data credited under `public/geo/` below (its red and green channels only); since round 57.9 (moonlight) the coast and the sea beyond that grid are Natural Earth's 1:10m land (`scripts/data/ne10m-land-nyc.json`, public domain, below), the land is drawn in flat average tones (measured box means of the live map's own frames, no pixels of it) and passed through the live map's CSS night grade (`components/home/g3d/night.ts`); built-up land is told by the density of our own listings (`/api/lights`); the lights are our own listing coordinates drawn with the live layer's own glyph and planner. No night-light imagery (the asset's blue channel, NASA) and no map imagery of anyone's is in it | RealtyLT | Our own artwork | -- |
| public/images/home-g3d-cover-tall.webp | The same Google-camera cover for the phone, from that map's phone hero camera (780 x 1688), `--narrow=1` for the phone's glyph and floor (was `home-night-cover-tall.webp` until round 57.13) | RealtyLT | Our own artwork | -- |
| public/images/home-dusk-cover.webp | The real map's load cover at a laptop's width, A, the default in round 57.2 and since round 57.6 only a comparison (`?cover=dusk`): the land at dusk drawn by `scripts/make-map-cover.mjs --look=dusk` from the map's own hero camera, the relief and water from the elevation data credited under `public/geo/` below, the towns' glow from the same asset's night channel, and the homes' light from `home-night-poster.webp` laid over it. No map imagery of anyone's is in it | RealtyLT | Our own artwork | -- |
| public/images/home-dusk-cover-tall.webp | The same cover for the phone, from the phone's own hero camera (780 x 1688); its lights are the night flight's still re-shot from that camera by `scripts/make-night-poster.mjs --width=780 --height=1688` | RealtyLT | Our own artwork | -- |
| public/images/home-day-cover.webp | The real map's load cover B, a comparison only (`?cover=day` or `NEXT_PUBLIC_HOME_COVER=day`): the same land by day, `scripts/make-map-cover.mjs --look=day`, hill-shaded from the elevation data credited under `public/geo/` below. Not a screenshot of any map | RealtyLT | Our own artwork | -- |
| public/images/home-day-cover-tall.webp | Cover B for the phone, from the phone's hero camera (780 x 1688) | RealtyLT | Our own artwork | -- |
...
| `scripts/data/ne10m-land-nyc.json` (round 57.9) | The land polygons of the region, clipped to the load covers' footprint (39.8 to 44.2 N, 76.8 to 71.2 W), lng/lat to 4 decimals: the coast and the sea beyond the elevation grid in the real map's night cover (`scripts/make-map-cover.mjs`) | Natural Earth 1:10m physical vectors, `ne_10m_land`, from `github.com/nvkelso/natural-earth-vector` (`geojson/ne_10m_land.geojson`, master), fetched 2026-09-24 | Public domain (naturalearthdata.com/about/terms-of-use: no permission or attribution required; named in the footer anyway, "Coastline: Natural Earth.") |
...
- NASA Media Usage Guidelines, `nasa.gov/nasa-brand-center/images-and-media/`, read 2026-09-22: "NASA
  ...
  2016, Suomi NPP VIIRS)." Since round 57.6 that line shows only when the home page's ground is the
  night flight (no Maps key, or `NEXT_PUBLIC_HOME_MAP=night`), whose scene uses the night-light
  channel; the real map's ground and its night cover use none of it (`components/site/SceneCredit.tsx`).
