...
| public/images/home-night-poster.webp | The home page's first screen with no JavaScript and no WebGL: one frame of the night-flight scene's own establishing shot, rendered from the running site by `scripts/make-night-poster.mjs` (round 54). The lights are our own listing coordinates; the land is the elevation data credited under `public/geo/` below | RealtyLT | Our own artwork | -- |
